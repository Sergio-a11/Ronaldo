...
CREATE TABLE inventario(
	...
	id_mercancia varchar(3) not null,
    tipo_mercancia varchar(20) not null,
    cantidad int not null,
    foreign key (id_mercancia) references producto(id_p),
    foreign key (id_mercancia) references materia_prima(id_mp),
    foreign key (id_mercancia) references corte(id_corte)
);
